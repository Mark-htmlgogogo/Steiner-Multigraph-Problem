...
MaxIterPool = ["1", "3", "5"]
TimeLimitPool = ["5", "10", "15"]
R_deltaPool = ["2", "5", "10"]

for LB_MaxRestart in MaxIterPool:
    LB_MaxIter = 2*LB_MaxRestart
    for BCTime in TimeLimitPool:
        for R_delta in R_deltaPool:
            for i in range(1, int(graph_number)+1):
                ...
                subprocess.Popen([exeAbsltLocation, tempDataLocation, formulation, callback_option, relax_option,
                                  ...
                                  LB_CP_Option, lazy_sep_opt, R_delta]).wait()
