...
void SmpSolver::build_problem_ns() {
    ...
    //                     int N = XYtoN[NODE_PAIR(u, v)];
    // 					if (u >= 1 && u <= MOD && v >= 1 && v <=
    // MOD) { 						if (vis[N]) continue;
    // if (!subG.node_value().count(N)) continue;
    // 						// cout << N << " ";
    // 						Ex.push_back(N);
    // 						vis[N] = 1;
    // 						if ((N == 1 && t != 1) || (t != MOD * MOD && N == MOD *
    // MOD)) { 							flag = true;
    // 						}
    // 					}
    //                 }
...
void SmpSolver::print_to_file() {
	...
	while (store[store.size() - 1] != '\\') { 
		runfilename.push_back(*store.rbegin());
		store.pop_back(); 
	}
	std::reverse(runfilename.begin(), runfilename.end());
    switch (formulation) {
        case SCF: {
            store = store + "1_SCF";
            break;
    ...
    flow << setw(LSPACING) << runfilename;  // graph number
