...
SmpSolver::SmpSolver(IloEnv env, std::shared_ptr<Graph> g_ptr,
    ...
    // cplex.setParam(IloCplex::TreLim,12288);  // set the limit of tree memory
    // in megabytes
...
void SmpSolver::update_problem(const const map<NODE, double>& obj_coeff,
    ...
    /* if (formulation == NS) {
                                                                                                                                                                                                                                                                    double M =
    0; IloExpr sigma_vars(env); for (auto k : G->p_set()) { SUB_Graph subG =
    G->get_subgraph()[k]; pair<NODE, INDEX> pair_i_k; for (auto i :
    subG.nodes()) { pair_i_k.first = i; pair_i_k.second = k; sigma_vars +=
    partition_node_vars[pair_i_k]; M += 1.0;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                    totalCost +=
    ((1.0 / (M + 1.0)) * sigma_vars);
    } */
...
void SmpSolver::solve() {
    ...
    /*for (auto var : primal_node_vars)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    cout <<
       var.second.getName() << "\t" << cplex.getValue(var.second)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    << endl;*/
...
void SmpSolver::build_problem_ns() {
    ...
    vector<IloConstraint> neighborConstraints1;
    vector<IloConstraint> neighborConstraints2;
    ...
    for (auto u : G->nodes()) {
        ...
        IloConstraint e = (sigma_vars >= 2 * primal_node_vars[u]);
        neighborConstraints1.push_back(e);
    }
    ...
    for (auto k : G->p_set()) {
        ...
        //* add cons 5, choose one of the neighbor of terminal to be true
        for (auto t : subG.t_set()) {
            IloExpr sigma_vars(env);
            ...
            IloConstraint e = (sigma_vars >= 1);
            neighborConstraints2.push_back(e);
        }
    ...
    //! equal ratio added
    // int totCons = neighborConstraints1.size() + neighborConstraints2.size();
    // int requiredCons1 = (int)floor(neighborConstraints1.size() *
    // neighborCoeff); int requiredCons2 =
    // (int)floor(neighborConstraints2.size() * neighborCoeff); int added1 = 0;
    // int added2 = 0;
    // while (added1 < requiredCons1) {
    //     model.add(neighborConstraints1[added1]);
    //     added1++;
    // }
    // while (added2 < requiredCons2) {
    //     model.add(neighborConstraints2[added2]);
    //     added2++;
    // }
    // addedNieghborConstriants = added1 + added2;
    // cout << " Coefficients: " << neighborCoeff << endl
    //      << " Total Cons: " << addedNieghborConstriants << endl
    //      << " added Cons: " << added1 << added2 << endl;

    //! random added
    vector<IloConstraint> neighborConstraint;
    for (auto e : neighborConstraints1) neighborConstraint.push_back(e);
    for (auto e : neighborConstraints2) neighborConstraint.push_back(e);

    auto get_random_num = [&](int a, int b) {
        return ((int)rand() % (b - a)) + a;
    };

    int requiredCons = (int)floor(neighborConstraint.size() * neighborCoeff);
    int added = 0;
    while (added < requiredCons) {
        int pos = get_random_num(0, neighborConstraint.size());
        model.add(neighborConstraint[pos]);
        neighborConstraint.erase(neighborConstraint.begin() + pos);
        added++;
    }
    addedNieghborConstriants = added;
    cout << " Coefficients: " << neighborCoeff << endl
         << " Total Cons: "
         << neighborConstraints1.size() + neighborConstraints2.size() << endl
         << " added Cons: " << addedNieghborConstriants << endl;

    //* Add cut pool constraint
    /*int CutPoolSize = cutpool.cutPoolLhs().size();
    for (int i = 0; i < CutPoolSize; i++) {
                                                                                                                                                                                                                                                                    model.add(cutpool.cutPoolLhs()[i]
    >= 1);
    }*/
...
void SmpSolver::print_to_file() {
    ...
    flow << setw(LSPACING) << fixed << cplex.getObjValue();
    flow << setw(LSPACING) << cplex.getStatus();
    flow << setw(LSPACING) << addedNieghborConstriants;
...
void LBSolver::GenerateInitialSolution(int k) {
    ...
    /*for (SmartDigraph::NodeIt n(g); n != INVALID; ++n) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    std::cout <<
    rev_nodes[n] << std::endl;
    }
    cout << endl;
    for (SmartDigraph::ArcIt a(g); a != INVALID; ++a) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    cout <<
    rev_nodes[g.source(a)] <<" "<< rev_nodes[g.target(a)] << endl;
    }*/
...
void LBSolver::LocalBranch(int& ObjValue, double& gap) {
    ...
    while (Iter++ <= LB_MaxIter && R <= Rmax) {
        ...
        /*for (auto k : G->p_set()) {
                                                                                                                                                                                                                                                                        SUB_Graph
        subG = G->get_subgraph()[k]; IloExpr sigma_vars(env); for (auto i :
        xPartSol[k]) { if (i.second == 0) continue; pair_i_k.first = i.first;
        pair_i_k.second = k; sigma_vars
        += (1 - partition_node_vars[pair_i_k]);
                                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                                        IloConstraint
        cons = sigma_vars <= R; LBmodel.add(cons); cons_array.add(cons);
        }*/
        ...
        /*for (auto k : G->p_set()) {
                                                                                                                                                                                                                                                                        pair_i_k.second
        = k; for (auto i : xPartSol[k]) { pair_i_k.first = i.first;
        VarArray.add(partition_node_vars[pair_i_k]); if (i.second) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        NumArray.add(1);
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        else {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        NumArray.add(0);
        ...
        try {
                                                                                                                                                                                                                                                                        cout <<
        "Objectvie value \t= \t" << LBcplex.getObjValue() << endl;
        }
        catch (IloException e) {
                                                                                                                                                                                                                                                                        cout <<
        e << endl;
        ...
        if ((LBcplex.getStatus() == IloAlgorithm::Feasible ||
            ...
            for (auto k : G->p_set()) {
                                                                                                                                                                                                                                                                            subG
            = G->get_subgraph()[k]; pair_i_k.second = k; for (auto i :
            subG.nodes()) { pair_i_k.first = i; xPartSol[k][i] =
            val[x_varindex_ns[pair_i_k]];
                                                                                                                                                                                                                                                                            }
            }*/
            for (auto i : G->nodes()) {
...
void LBSolver::FinalSolve() {
    ...
    switch (callbackOption) {
        ...
        case 1:
            FLBcplex.use(NS_StrongComponentLazyCallback(
                nenv, G, primal_node_vars, x_vararray, x_varindex_ns, tol_lazy,
                max_cuts_lazy, formulation, ns_root, x_vararray_primal,
                x_varindex_ns_primal, lazy_sep_opt));
            break;
        case 2:
            FLBcplex.use(NS_CutCallback(
                nenv, G, primal_node_vars, x_vararray, x_varindex_ns, tol_user,
                max_cuts_user, formulation, ns_root, ns_sep_opt, LB_CP_Option,
                fianlsolveflag, lazy_sep_opt, x_vararray_primal,
                x_varindex_ns_primal));
            break;
        case 3:
            FLBcplex.use(NS_StrongComponentLazyCallback(
                nenv, G, primal_node_vars, x_vararray, x_varindex_ns, tol_lazy,
    ...
    /*for (auto k : G->p_set()) {
                                                                                                                                                                                                                                                                    pair_i_k.second
    = k; for (auto i : Final_xPartSol[k]) { pair_i_k.first = i.first;
    VarArray.add(partition_node_vars[pair_i_k]); if (i.second) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    NumArray.add(1);
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    else {
    ...
    for (auto i : G->nodes()) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    sigma_vars
    += G->nodes_value().at(i)*primal_node_vars[i];
    }
    ...
    */
    ...
    FLBcplex.setParam(IloCplex::MIPDisplay,
                      MIPDisplayLevel);  // set display level
    if (formulation > 0)
        FLBcplex.setParam(IloCplex::AdvInd, 1);  // start value: 1
    FLBcplex.setParam(IloCplex::EpGap, 1e-09);   // set MIP gap tolerance
    FLBcplex.setParam(IloCplex::Threads,
                      8);  // set the number of parallel threads
    // FLBcplex.setParam(IloCplex::TreLim, 12288);  // set the limit of tree
    // memory in megabytes
    FLBcplex.setParam(IloCplex::TiLim,
                      3600);  // set time limit in secs, default:1200
