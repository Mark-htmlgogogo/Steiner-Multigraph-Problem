...
void SmpSolver::update_problem(const const map<NODE, double>& obj_coeff,
    ...
    /* if (formulation == NS) {
                                    ...
                                    for (auto k : G->p_set()) {
                                                                    SUB_Graph
    subG = G->get_subgraph()[k]; pair<NODE, INDEX> pair_i_k; for (auto i :
    ...
    while (Iter++ <= LB_MaxIter && R <= Rmax) {
        ...
        /*for (auto k : G->p_set()) {
                                        ...
                                        for (auto i : xPartSol[k]) {
                                                                        if
        (i.second == 0) continue; pair_i_k.first = i.first; pair_i_k.second = k;
                                                                        sigma_vars
        ...
        /*for (auto k : G->p_set()) {
                                        ...
                                        for (auto i : xPartSol[k]) {
                                                                        pair_i_k.first
        = i.first; VarArray.add(partition_node_vars[pair_i_k]); if (i.second) {
                                                                                                        NumArray.add(1);
                                                                        }
                                                                        else {
                                                                                                        NumArray.add(0);
        ...
        try {
                                        cout << "Objectvie value \t= \t" <<
        LBcplex.getObjValue() << endl;
        }
        catch (IloException e) {
                                        cout << e << endl;
        }
        ...
        if ((LBcplex.getStatus() == IloAlgorithm::Feasible ||
            ...
            for (auto k : G->p_set()) {
                                            ...
                                            for (auto i : subG.nodes()) {
                                                                            pair_i_k.first
            = i; xPartSol[k][i] = val[x_varindex_ns[pair_i_k]];
                                            }
            }*/
            for (auto i : G->nodes()) {
