...
void SmpSolver::build_problem_ns() {
	...
	vector<IloConstraint> neighborConstraints;
	...
	for (auto u : G->nodes()) {
		...
		neighborConstraints.push_back(e);
	}
	...
	//* add partial neighbors constraints
	auto get_random_num = [&](int a, int b) {
		return ((int)rand() % (b - a)) + a;
	};

	int totCons = neighborConstraints.size();
	int requiredCons = (int)floor(neighborConstraints.size() * neighborCoeff);
	int added = 0;
	int pos = 0;
	while (added < requiredCons) {
		pos = get_random_num(0, neighborConstraints.size());
		//model.add(neighborConstraints[pos]);
		neighborConstraints.erase(neighborConstraints.begin() + pos);
		added++;
	}
	cout << " Coefficients: " << neighborCoeff << endl
		<< " Total Cons: " << totCons << endl
		<< " required Cons: " << requiredCons << endl
		<< " added Cons: " << added << endl;
