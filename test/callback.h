...
public:
	...
	StrongComponentLazyCallbackI(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE_PAIR, INDEX>, IloNumVar>edge_vars_,
	                             IloNumVarArray x_vararray_, map<pair<NODE_PAIR, INDEX>, int>x_varindex_Steiner_, double tol_lazy_, int max_cuts_lazy_, SmpForm form_,
	                             map<INDEX, NODE>root_, map<NODE, IloNumVar>primal_node_vars_)
		: IloCplex::LazyConstraintCallbackI(env), G(graph), edge_vars(edge_vars_), x_vararray(x_vararray_), 
											x_varindex_Steiner(x_varindex_Steiner_), tol_lazy(tol_lazy_), max_cuts_lazy(max_cuts_lazy_), form(form_),
											root(root_), primal_node_vars(primal_node_vars_) {}
...
IloCplex::Callback StrongComponentLazyCallback(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE_PAIR, INDEX>, IloNumVar>edge_vars,
        IloNumVarArray x_vararray, map<pair<NODE_PAIR, INDEX>, int>x_varindex_Steiner, double tol_lazy, int max_cuts_lazy, SmpForm form,
        map<INDEX, NODE>root, map<NODE, IloNumVar> primal_node_vars)
{
	return (IloCplex::Callback(new (env) StrongComponentLazyCallbackI(env, graph, edge_vars, x_vararray, x_varindex_Steiner,
	                           tol_lazy, max_cuts_lazy, form, root, primal_node_vars)));
}
...
public:
	...
	SmpCutCallbackI(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE_PAIR, INDEX>, IloNumVar>edge_vars_,
	                IloNumVarArray x_vararray_, map<pair<NODE_PAIR, INDEX>, int>x_varindex_Steiner_, double tol_user_, int max_cuts_user_, SmpForm form_,
	                map<INDEX, NODE>root_, map<NODE, IloNumVar>primal_node_vars_)
		: IloCplex::UserCutCallbackI(env), G(graph), edge_vars(edge_vars_), x_vararray(x_vararray_), x_varindex_Steiner(x_varindex_Steiner_),
		  tol_user(tol_user_), max_cuts_user(max_cuts_user_), form(form_), root(root_), primal_node_vars(primal_node_vars_) {}

	void main();
};

IloCplex::Callback SmpCutCallback(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE_PAIR, INDEX>, IloNumVar>edge_vars,
                                  IloNumVarArray x_vararray, map<pair<NODE_PAIR, INDEX>, int>x_varindex_Steiner, double tol_user, int max_cuts_user, SmpForm form,
                                  map<INDEX, NODE>root, map<NODE, IloNumVar> primal_node_vars)
{
	return (IloCplex::Callback(new (env) SmpCutCallbackI(env, graph, edge_vars, x_vararray, x_varindex_Steiner,
	                           tol_user, max_cuts_user, form, root, primal_node_vars)));
}
...
public:
	...
	NS_StrongComponentLazyCallbackI(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE, INDEX>, IloNumVar>partition_node_vars_,
	                                IloNumVarArray x_vararray_, map<pair<NODE, INDEX>, int>x_varindex_ns_, double tol_lazy_, int max_cuts_, SmpForm form_, map<INDEX, NODE> ns_root_)
		: IloCplex::LazyConstraintCallbackI(env), G(graph), partition_node_vars(partition_node_vars_), x_vararray(x_vararray_), x_varindex_ns(x_varindex_ns_),
		  tol_lazy(tol_lazy_), max_cuts(max_cuts_), form(form_), ns_root(ns_root_) {}
...
IloCplex::Callback NS_StrongComponentLazyCallback(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE, INDEX>, IloNumVar>partition_node_vars,
        IloNumVarArray x_vararray, map<pair<NODE, INDEX>, int>x_varindex_ns, double tol_lazy, int max_cuts, SmpForm form, map<INDEX, NODE> ns_root)
{
	return (IloCplex::Callback(new (env) NS_StrongComponentLazyCallbackI(env, graph, partition_node_vars, x_vararray, x_varindex_ns, tol_lazy, max_cuts, form, ns_root)));
}
...
public:
	...
	NS_CutCallbackI(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE, INDEX>, IloNumVar>partition_node_vars_,
	                IloNumVarArray x_vararray_, map<pair<NODE, INDEX>, int>x_varindex_ns_, double tol_user_, int max_cuts_user_, SmpForm form_,
		...
		: IloCplex::UserCutCallbackI(env), G(graph), partition_node_vars(partition_node_vars_), x_vararray(x_vararray_), x_varindex_ns(x_varindex_ns_),
		  tol_user(tol_user_), max_cuts_user(max_cuts_user_), form(form_), ns_root(ns_root_), ns_sep_opt(ns_sep_opt_) {}

	void main();
};

IloCplex::Callback NS_CutCallback(IloEnv env, std::shared_ptr<Graph>graph, map<pair<NODE, INDEX>, IloNumVar>partition_node_vars,
                                  IloNumVarArray x_vararray, map<pair<NODE, INDEX>, int>x_varindex_ns, double tol_user, int max_cuts_user, SmpForm form,
                                  map<INDEX, NODE> ns_root, bool ns_sep_opt)
{
	return (IloCplex::Callback(new (env) NS_CutCallbackI(env, graph, partition_node_vars, x_vararray, x_varindex_ns, tol_user, max_cuts_user, form, ns_root, ns_sep_opt)));
}
...
void NS_CutCallbackI::main()
{
	...
	if (ns_sep_opt) {
		if (!seperate_sc_ns(masterEnv, xSol, G, partition_node_vars, cutLhs, cutRhs, violation, ns_root))
