...
bool seperate_sc_ns(
    ...
    for (auto k : G->p_set()) {
        ...
        for (auto target_set : comp_set) {
            ...
            // cout << "newCutValue = " << newCutValue << endl;
            // cout << "new Violation = " << newViolation << endl;
            ...
            if (min_cut_value < 1 - TOL) {
                ...
                /*for (ListDigraph::NodeIt i(cap_graph); i != INVALID; ++i)
                {
                                if (min_cut.minCut(i))
                                {
                                                ...
                                                ListNode a =
                v_nodes[rev_nodes[i]].first; ListNode b =
                v_nodes[rev_nodes[i]].second; if (a == i) is_cut_arc =
                !min_cut.minCut(b); else if (b == i) is_cut_arc =
                !min_cut.minCut(a); if (is_cut_arc) { LOG << "find cur arc: " <<
                rev_nodes[i]
