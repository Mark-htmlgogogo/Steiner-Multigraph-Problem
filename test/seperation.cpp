...
void generate_ns_mincut_graph(std::shared_ptr<Graph> G, const map<INDEX, NODE>& ns_root) {
	...
	for (auto k : G->p_set()) {
		...
		for (NODE i : subG.nodes()) {
			if (i == ns_root.at(k)) continue;
			ListNode new_node = ns_mincut_capgraph[k].split(ns_mincut_v_nodes[k][i].first, false);
	...
	for (auto k : G->p_set()) {
		...
		for (auto q : subG.t_set()) {
			...
			Preflow<ListDigraph, ListDigraph::ArcMap<double>> min_cut(
				ns_mincut_capgraph[k], x_capacities, ns_mincut_v_nodes[k][ns_root.at(k)].first,
